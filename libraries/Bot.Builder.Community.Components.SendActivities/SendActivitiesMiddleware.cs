...
{
    public class SendActivitiesMiddleware : IMiddleware
    {
        ...
        public SendActivitiesMiddleware(string appId, string appPassword)
        {
            this.appId = appId;
                ...
                if (activities.Count == 1 && !String.IsNullOrEmpty(activities[0].DeliveryMode))
                {
                    ...
                    switch (activity.DeliveryMode)
                    {
                        ...
                        case DeliveryModesPlus.Delete:
                            ...
                            break;
                        case DeliveryModesPlus.DirectMessage:
                            activity.DeliveryMode = null;
                            if (ctx.Activity.ChannelId != "emulator")
                            {
                                return await this.SendDirectMessageAsync(ctx, activity, cancellationToken).ConfigureAwait(false);
                            }
                            else
                            {
                                response = await retryPolicy.ExecuteAsync(() => ctx.SendActivityAsync(activity, cancellationToken)).ConfigureAwait(false);
        ...
        private async Task<ResourceResponse[]> SendDirectMessageAsync(ITurnContext context, Activity activity, CancellationToken cancellationToken)
        {
